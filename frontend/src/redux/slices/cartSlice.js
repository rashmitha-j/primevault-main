...
export const fetchCart  = createAsyncThunk("cart/fetchCart", async({userId, guestId} ,
    ...
    export const addToCart = createAsyncThunk("cart/addToCart", async({productId, quantity, size, color,guestId, userId}
        ...
        export const clearCartInDB = createAsyncThunk("cart/clearCartInDB", async ({ userId, guestId }, { rejectWithValue }) => {
            try {
              const res = await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/cart/clear`, {
                data: { userId, guestId },
              });
              return res.data;
            } catch (err) {
